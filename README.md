...
1、【要求】组件名要求用大驼峰的形式 例如：MenuButton、DatePicker 等
2、【要求】demo 名称用英文，demo 描述中通过如下格式来进行额外说明，其中描述的基本信息必须放置在文件的开头
```
---
title: Affix
subtitle: 固钉
...
组件示例对应的prompt，用来描述该示例

```


3、【建议】组件 API 文档推荐形式
传统的 API 文档在实现的时候，使用的是 MarkDown 中的表格形式，对于 AI 而言不是很友好，所以建议用来 AI 生成的 API 文档描述将表格拆解，具体展示如下：
...
16. **icons**
    ...
    - **默认值**: `{}`
    - **描述**: 提供自定义图标，可用于替换默认加载图标。
    - **应用场景**: 应用于需要定制图标的场景，如品牌一致化的设计。
...
```
4、【要求】仓库中必须有 package.json 文件，导入的组件版本、仓库名称都会从 package.json 中获取
5、【要求】请在组件使用文档中描述应该如何引用组件，因为承载很多的组件，所以需要说明组件应该如何使用，避免 AI 生成时引用组件错误##